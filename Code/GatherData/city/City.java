...
public class City{
	...
	/**
	 * Used in updateAggregate() to show more information
	 * 
	 * DON'T DELETE!!!!!!!!
	 */
	 ...
	 * DON'T DELETE!!!!!!!!
	 */
	public double getDistancePopulation(){
		double distance = 0;
		int times = 0;
		for(Cell c:City.urbanCells){
			if(c.getCity_ID()==this.city_ID)
				distance += Math.abs((c.getLocation().getX() - this.getCentreX())) + Math.abs((c.getLocation().getY() - this.getCentreY()));
				times++;
		}
