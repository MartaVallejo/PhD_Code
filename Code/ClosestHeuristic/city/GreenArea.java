...
public class GreenArea{
	 ...
	 */
	public GreenArea(Context context, City[] city){
	 ...
	 */
	public int searchGreenSpaceS(int budget, int tick){
		int restBudget = budget;
		Cell selectedCell = null;
		boolean isFound = false;
		
		int index = RandomHelper.nextIntFromTo(0, Lattice.NUM_CBDS-1);
		for(Cell c: city[index].getLastUrbanised().getNeighbours()){
			if(!isFound){
				if(c.isEmpty() && c.getPrice()<restBudget){
					System.err.println("Protecting: " + c + " price:" + c.getPrice());
					selectedCell=c;
					isFound= true;	
					restBudget -= selectedCell.getPrice();
					//System.err.println("Budget:" + budget + " price:" + c.getPrice() + " rest budget:" + restBudget + " Cell state:" + c.getStateString());
					...
					if(greenAreas.length()==2)
						greenAreas = greenAreas.substring(0,1) + c.getLocation().getX() + ":" + c.getLocation().getY() + "]";
					else
						greenAreas = greenAreas.substring(0,greenAreas.length()-1) + "-" + c.getLocation().getX() + ":" + c.getLocation().getY() + "]";
				}
			}
		}
		return restBudget;
	 ...
	 * @return the method returns the remained budget.
	 */
	public int searchGreenSpaceP(int budget, int tick){
		int restBudget = budget;
		Cell selectedCell = null;
		boolean isFound = false;
		...
		while(attempts >0){
			//If no protected cell -> like random
			if(GreenArea.protectedCells.size()==0 || randnum.nextBoolean()==true){
				...
				if(selectedCell!=null){
					if(selectedCell.getPrice()<restBudget){
				...
				int index = RandomHelper.nextIntFromTo(0, GreenArea.protectedCells.size()-1);
				for(Cell c: GreenArea.protectedCells.get(index).getNeighbours()){
					if(!isFound){
						if(c.isEmpty() && c.getPrice()<restBudget){
							System.err.println("Protecting: " + c + " price:" + c.getPrice());
							selectedCell=c;
